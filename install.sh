...
_SYSTEM_BIN_LINK="/usr/bin/local_tvcmd"
...
echo_desktop_file(){
    ...
    echo -e "Name=local_tvcmd"
    echo -e "GenericName=local_tvcmd"
    echo -e "Icon=tv-symbolic"
    echo -e "Exec=sh ${INSTALL_DIR}/tvcmd_remote.sh"
...
if [[ "${MODE}" == "install" ]]; then
    mkdir -p "${INSTALL_DIR}" || (echo "error installation: 'mkdir -p ${SCRIPT_DIR_PATH}' failed…"; exit 5)
    cp -rvf "${SCRIPT_DIR_PATH}"/{tvcmd_remote.sh,worker.sh,tvcmd_wrapper.sh,GUI.py,gui.glade} "${INSTALL_DIR}"

    echo_desktop_file > "${APP_DIR}/local_tvcmd.desktop"

    if [[ "${INSTALL_DIR}" == "${SYSTEM_INSTALL_DIR}" ]]; then
        ln -s "${_SYSTEM_BIN_LINK}" "${INSTALL_DIR}/tvcmd_remote.sh"
    fi
    ...
    rm -rvf "${INSTALL_DIR}" "${APP_DIR}/local_tvcmd.desktop"
fi
